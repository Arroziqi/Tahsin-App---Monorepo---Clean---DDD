...
@Injectable()
export class AddLevelUsecase implements UseCase<LevelEntity, DataState<LevelEntity>> {
  private readonly logger = new Logger(AddLevelUsecase.name);

  constructor(@Inject(LEVEL_REPO_TOKEN) private readonly levelRepository: LevelRepository) { }
  async execute(input: LevelEntity): Promise<DataState<LevelEntity>> {
    const existingLevel = await this.levelRepository.findByName(input.name, true);
    ...
    if (existingLevel.data && existingLevel.data.id) {
      this.logger.warn(`Create level attempt with existing name: ${input.name}`);
      throw new DataFailed<LevelEntity>(new ErrorEntity(409, "Level already exists"));
    }
